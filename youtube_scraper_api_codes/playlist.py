"""
YouTube Playlist - Chocodata YouTube Scraper API
...
Runnable example. It calls the LIVE API and prints the real JSON response.
    ...
    python youtube_scraper_api_codes/playlist.py
...
API = "https://api.chocodata.com/api/v1/youtube/playlist"
...
def playlist(playlist_id: str) -> dict:
    """A playlist's videos plus its header metadata."""
    params = {"api_key": KEY, "playlist_id": playlist_id}
...
if __name__ == "__main__":
    data = playlist("PLWKjhJtqVAbkmRvnFmOd4KhDdlK1oIq23")
    preview = {k: v for k, v in data.items() if k != "videos"}
    print(json.dumps(preview, indent=2, ensure_ascii=False))
    print()
    for v in data["videos"][:5]:
        print(f"  {v['position']:2}. {v['title'][:55]:55} {str(v['views']):>12}")
    print(f"\n{data['title']} | {data['channel']} | {data['videos_returned']} of "
          f"{data['video_count']} videos | {data['views']:,} views")
